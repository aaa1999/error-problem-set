...
技术栈：桌面端 Tauri 2 + React + TypeScript + Vite；iOS 端原生 SwiftUI（`ios/` 目录，数据格式与桌面端完全互通，详见 [docs/ios.md](./docs/ios.md)）。设计文档见 [DESIGN.md](./DESIGN.md)。
...
- **翻页浏览**：一页一题，解析默认遮罩（先想再看），左右键/触屏滑动翻页，图片点击放大；题目**可按录入时间、错误率或随机排列**（随机适合复习防背序，可随时重新洗牌换一批）
