...
顶栏「☁ 同步」把整库推送到自建服务器（填 `ip:端口`，可选「记住此地址」，随时改填新的）：

- **增量上传**：图片按内容哈希命名，先向服务器查询已有图片清单，**只传缺的**；`data.json` 整份推送，几 KB 秒传
- **幂等可续传**：中断后重新同步，已传过的自动跳过；可随时重复同步
- 支持可选访问令牌（`X-Sync-Token`）；同步中显示逐张进度，可中止
- 服务端只需 3 个 HTTP 端点，接口文档（含 curl 示例）见 **[docs/sync-protocol.md](./docs/sync-protocol.md)**；仓库自带零依赖 Python 实现 **[backend/sync_server.py](./backend/sync_server.py)**（部署说明见 [backend/README.md](./backend/README.md)），一条命令即可起服务
