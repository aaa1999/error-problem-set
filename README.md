...
- **数据目录合并**：把另一份错题本数据目录（data.json + assets，如旧备份、其他设备的数据）整体合并进来——题目/文件夹/标签全量迁移，文件夹同名自动合并，图片按内容哈希去重，已导过的自动跳过，可重复执行
- **翻页浏览**：一页一题，解析默认遮罩（先想再看），左右键/触屏滑动翻页，图片点击放大
- **文件夹管理**：浏览页左侧侧栏，多层文件夹树（新建/重命名/删除/新建子文件夹），按文件夹浏览（含子文件夹），当前题可一键「移动」
- **多标签**：一道错题可打多个标签；侧栏按标签多选筛选（可切换交集/并集），文件夹与标签筛选叠加生效、计数双向联动；浏览页/录入页均可新增或点选已有标签
